...
public class GameTestBlocks {
    ...
    public static final PulseStateBlock PULSE_STATE_BLOCK = register(
            "pulse_state_block",
            new PulseStateBlock(FabricBlockSettings.copyOf(Blocks.STONE).pushReaction(BLOCK))
    );
    public static final CheckStateBlock CHECK_STATE_BLOCK = register(
            "check_state_block",
            new CheckStateBlock(FabricBlockSettings.copyOf(Blocks.STONE).pushReaction(BLOCK))
    );
    public static final TestTriggerBlock TEST_TRIGGER_BLOCK = register(
            "test_trigger_block",
            new TestTriggerBlock(FabricBlockSettings.copyOf(Blocks.WHITE_CONCRETE).pushReaction(BLOCK))
    );
    public static final GameTestPoweredBlock GAMETEST_REDSTONE_BLOCK = register(
            "gametest_redstone_block",
            new GameTestPoweredBlock(FabricBlockSettings.copyOf(Blocks.REDSTONE_BLOCK).pushReaction(BLOCK))
    );
    public static final EntityInteractionBlock ENTITY_INTERACTION_BLOCK = register(
            "entity_interaction_block",
            new EntityInteractionBlock(FabricBlockSettings.copyOf(Blocks.STONE).pushReaction(BLOCK))
    );
    public static final EntityInsideBlock ENTITY_INSIDE_BLOCK = register(
            "entity_inside_block",
            new EntityInsideBlock(FabricBlockSettings.copyOf(Blocks.STONE).collidable(false).pushReaction(BLOCK))
    );
